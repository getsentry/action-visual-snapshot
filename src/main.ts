...
const pngGlob = '/**/*.png';
const shouldSaveOnly = process.env.ACTION_SAVE_ONLY || core.getInput('save-only');
...
async function run(): Promise<void> {
  ...
  const snapshotPath: string = process.env.ACTION_SNAPSHOT_PATH || core.getInput('snapshot-path');
  ...
  try {
    if (snapshotPath && !process.env.ACTION_DISABLE_SAVE) {
      await saveSnapshots({
        artifactName,
